...
bool GLExample::init() {
  ...
  programForTexturedShape = std::make_shared<ShaderProgram>(
      std::string(SHADER_DIR) + "/TexturedShape");
  programForTexturedNormals = std::make_shared<ShaderProgram>(
      std::string(SHADER_DIR) + "/TexturedNormals");
  programForLightBox =
      std::make_shared<ShaderProgram>(std::string(SHADER_DIR) + "/Lightbox");
  programForBasicShape =
      std::make_shared<ShaderProgram>(std::string(SHADER_DIR) + "/BasicShape");
  programForBasicNormal = std::make_shared<ShaderProgram>(
      std::string(SHADER_DIR) + "/BasicNormals");
  ...
  normalsTorus = std::make_shared<MultiLine>(torus->positions, torus->normals,
                                             torus->tangents, torus->texCoords);
   ...
   *      texture for the cube. Don't forget to initialize the shared pointers
   * existing already in this class for this purpose.
   */
 ...
 * shaders. Don't forget that shader have to be bound before shader uniform can
 * be set The light color is available in lightColor
 */
void GLExample::addLightVariables(
  ...
  // Light source
  glm::vec3 light(lightColor[0], lightColor[1], lightColor[2]);
  glUniform3fv(program->getUniformLocation("lightPos"), 1,
               &lightbox->getPosition()[0]);

  // Ambient light
  float ambient_intensity = 0.4;
  // float ambient_intensity = 0.0;
  glm::vec3 ambient = ambient_intensity * light;
  glUniform3fv(program->getUniformLocation("ambient"), 1, &ambient[0]);

  // // Diffuse reflection
  // float surface_reflectance = 0.0;
  float surface_reflectance = 0.75;
  glm::vec3 diffuse = surface_reflectance * light;
  glUniform3fv(program->getUniformLocation("diffuse"), 1, &diffuse[0]);

  // Specular reflection
  float specular_reflectance = 0.75;
  // float specular_reflectance = 0.0;
  glm::vec3 specular = specular_reflectance * light;
  glUniform3fv(program->getUniformLocation("specular"), 1, &specular[0]);

  // Camera position
  glUniform3fv(program->getUniformLocation("camPos"), 1, &cam.getPosition()[0]);

  // Render mode
  glUniform1i(program->getUniformLocation("gouraudShading"), renderMode);

  // Shininess
  float shiny = 50.0;
  glUniform1f(program->getUniformLocation("shiny"), shiny);
}
// END TODO
...
void GLExample::renderBasicCube() {
  ...
  glUniformMatrix4fv(programForBasicShape->getUniformLocation("mvpMatrix"), 1,
                     GL_FALSE, &mvpMatrix[0][0]);
  glUniformMatrix4fv(programForBasicShape->getUniformLocation("modelMatrix"), 1,
                     GL_FALSE, &cube->getModelMatrix()[0][0]);
  glUniform1i(programForBasicShape->getUniformLocation("gouraudShading"),
              renderMode == GOURAUD_SHADING);
  cube->draw();
...
void GLExample::renderBasicTorus() {
  ...
  if (drawTorusNormals) {
    ...
    mvpMatrix = cam.getViewProjectionMatrix() * torus->getModelMatrix();
    glUniformMatrix4fv(programForBasicNormal->getUniformLocation("mvpMatrix"),
...
void GLExample::renderTexturedCube() {
  ...
  /* TODO Task 2 bind textures for shaders with glBindTexture and
   * glActiveTexture. check the OpenGL documentation to understand these
   * commands. Also, remember that you will have to set the uniform variables in
  ...
  glUniform1i(programForTexturedShape->getUniformLocation("cubetex"), 0);

  // End TODO

  mvpMatrix = cam.getViewProjectionMatrix() * cube->getModelMatrix();
  glUniformMatrix4fv(programForTexturedShape->getUniformLocation("modelMatrix"),
                     1, GL_FALSE, &cube->getModelMatrix()[0][0]);
  glUniformMatrix4fv(programForTexturedShape->getUniformLocation("mvpMatrix"),
                     1, GL_FALSE, &mvpMatrix[0][0]);
  cube->draw();

  /* TODO Task 2 unbind textures by setting all glBindTextures for all active
   * texture layers to zero.
   */
  glActiveTexture(GL_TEXTURE0);
...
void GLExample::renderTexturedTorus() {
  ...
  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_2D, cubetex->getTexHandle());
  glUniform1i(programForTexturedShape->getUniformLocation("torustex"), 0);

  // TODO END

  mvpMatrix = cam.getViewProjectionMatrix() * torus->getModelMatrix();
  glUniformMatrix4fv(programForTexturedShape->getUniformLocation("modelMatrix"),
                     1, GL_FALSE, &torus->getModelMatrix()[0][0]);
  glUniformMatrix4fv(programForTexturedShape->getUniformLocation("mvpMatrix"),
                     1, GL_FALSE, &mvpMatrix[0][0]);
  torus->draw();
  ...
  if (drawTorusNormals) {
    ...
    mvpMatrix = cam.getViewProjectionMatrix() * torus->getModelMatrix();
    glUniformMatrix4fv(
        programForTexturedNormals->getUniformLocation("mvpMatrix"), 1, GL_FALSE,
        &mvpMatrix[0][0]);
    normalsTorus->draw();

    /* TODO Task 2 unbind normal texture by setting glBindTexture for the active
     * texture layer to zero.
     */
