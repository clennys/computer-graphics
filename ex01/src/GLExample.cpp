...
bool GLExample::render() {
   ...
   *find the commands and apply the parameters. There are several resources
  ...
  // calculate model matrix for the cube object and set the uniform to store it
  // in the shader
  mvpMatrix = viewProjectionMatrix * cube->getModelMatrix();
  glUniformMatrix4fv(mvpUniformLocation, 1, GL_FALSE, &mvpMatrix[0][0]);
  cube->draw();

  /* TODO: Add here code to call the draw method of the torus similar to the
   * cube above */
